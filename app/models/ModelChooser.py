...
class ModelChooser:
    ...
    def findBestModel(self, x, y):
        ...
        self.score = float(grid_count.best_score_)
        elapsed_time = time.time() - start_time
        print("elapsed time w/ best model ", self.name, " is ", time.strftime("%H:%M:%S", time.gmtime(elapsed_time)), "with accuracy:", self.score)
